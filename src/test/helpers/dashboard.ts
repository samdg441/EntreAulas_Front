...
const MAPA_DASHBOARD: Record<string, string> = {
...
const PRIORIDAD_ROLES = ['admin', 'decano', 'coordinador', 'profesor', 'docente', 'estudiante']

export function dashboardParaUsuario(u: UsuarioDashboard): string {
  if (u.dashboard) return u.dashboard
  if (u.roles && u.roles.length > 0) {
    for (const role of PRIORIDAD_ROLES) {
      if (u.roles.includes(role)) return MAPA_DASHBOARD[role] || '/dashboard'
    }
  }
  const tipo = (u.tipo_usuario || '').toLowerCase()
  return MAPA_DASHBOARD[tipo] || '/dashboard'
}
