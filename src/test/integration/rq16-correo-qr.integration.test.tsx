...
/** RQ16 integración: smoke share-email desde el modal. */
describe('RQ16 integration — Correo QR', () => {
  beforeEach(() => {
    ...
    shareQrEvaluationsEmail.mockResolvedValue({ sentTo: 'a@b.com', totalLinks: 1 })
    vi.spyOn(window, 'alert').mockImplementation(() => {})
  })

  it('C3: enviar correo llama shareQrEvaluationsEmail', async () => {
    ...
    fireEvent.change(await screen.findByPlaceholderText('Destinatario'), {
      target: { value: 'a@b.com' },
    })
    fireEvent.click(screen.getByRole('button', { name: /^Enviar$/i }))
    await waitFor(() => {
      expect(shareQrEvaluationsEmail).toHaveBeenCalledWith(
        expect.objectContaining({ to: 'a@b.com', grupoIds: [1] })
      )
    })
  })
})
